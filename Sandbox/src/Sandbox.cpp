...
int main()
{
	...
	Ref<Window> window = CreateRef<Window>("SVO Raytracer", 800, 600, false);
	...
	Ref<Octree> octree = CreateRef<Octree>();
	octree->SampleRandom(4);
	...
	while (!window->ShouldClose())
	{
		...
		renderer->Render(camera->GetViewMatrix(), camera->GetProjectionMatrix(), window, octree);
